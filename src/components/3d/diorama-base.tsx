...
export function DioramaBase() {
  return (
    <group>
      {/* Main platform */}
      <mesh receiveShadow position={[0, -PLATFORM_H / 2, 0]}>
        <boxGeometry args={[PLATFORM_W, PLATFORM_H, PLATFORM_D]} />
        <meshStandardMaterial color="#e0ddd5" roughness={0.9} metalness={0.02} />
      </mesh>

      {/* Lawn — left */}
      <mesh receiveShadow position={[-7.5, 0.03, -0.5]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={1}>
      ...
      {/* Lawn — back */}
      <mesh receiveShadow position={[0, 0.03, -9]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={1}>
        <planeGeometry args={[PLATFORM_W - 2, 4]} />
        <meshStandardMaterial color="#7daa5e" roughness={0.95} metalness={0} polygonOffset polygonOffsetFactor={-1} polygonOffsetUnits={-1} />
      </mesh>

      {/* Lawn patch — slightly different shade for organic look */}
      <mesh receiveShadow position={[-5, 0.032, 2]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={1}>
      ...
      <mesh receiveShadow position={[6, 0.032, -3]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={1}>
      ...
      {/* Driveway */}
      <mesh receiveShadow position={[3.5, 0.05, 6.5]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={2}>
      ...
      {/* Sidewalk */}
      <mesh receiveShadow position={[0, 0.04, 5.5]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={2}>
      ...
      {/* Short path to door */}
      <mesh receiveShadow position={[-0.5, 0.045, 3.4]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={3}>
